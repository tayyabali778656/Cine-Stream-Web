  ...
  try {
      ...
      for (const item of filteredAnime) {
        ...
        addEntry({
          ...
          video: posterUrl ? {
            thumbnail:   posterUrl,
            title:       `Watch ${title} in Hindi Dubbed Online Free`,
          ...
          for (let s = 1; s <= maxSeasons; s++) {
            for (let e = 1; e <= 3; e++) {
              addEntry({
                ...
                lastmod:    TODAY,
                image:      posterUrl,
                imageTitle: `${title} S${s}E${e} in Hindi Dubbed`,
                imageCaption: `Watch ${title} Season ${s} Episode ${e} in Hindi Dubbed free on CineStream.`,
                video: posterUrl ? {
                  thumbnail:   posterUrl,
                  title:       `${title} Season ${s} Episode ${e} in Hindi Dubbed`,
                  description: `Watch ${title} S${s}E${e} in Hindi Dubbed online free in HD on CineStream.`,
